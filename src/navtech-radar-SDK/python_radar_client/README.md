...
Warning - these Python Radar SDK examples are not fully supported on Windows or Mac, nor are they disgned for use in a production envioronment. These examples are solely
for demonstration of what can be achieved with the Radar SDK, and hence behaviour of these examples is undefined in these scenarios. For details of why these examples
behave differently when not running on a Linux platform, please look at the differences in thread spawning behaviour for the multiprocessing module, for different
operating systems (Linux uses fork by default, Windows/Mac do not currently support fork)
...
This version of the SDK was developed on Ubuntu 24.04, using Python version 3.12.3. The SDK has been designed with backwards compatability in mind, so previous Python versions should also work.
...
This folder now contains a helper script (install_prerequisites.sh) to ensure that the correct versions of required packages are installed. Note - this installs the correct version of protobuf, but does not re compile the protobuf files. See "installing and building protobuf" for instructions on how to re compile the protobuf files.

To run the script, first make it executable using: chmod +x install_prerequisites.sh

And run with: ./install_prerequisites.sh

# Building protobuf files

This folder now contains a helper script (compile_protobuf_files.sh) to ensure the correct version of protobuf is installed, and to re build the protobuf files for python, using the protobuf compiler.

To run the script, first make it executable using: chmod +x compile_protobuf_files.sh

And run with: ./compile_protobuf_files.sh

You should see the correct version of protobuf being installed, and the individual protobuf files being compiled for use with Python.

# Troubleshooting

If the helper bash scripts fail to run after changing the execution permissions, then it is advisable to check that git has not changed the line endings from unix to dos format (it tends to do this automatically in some cases). The dos2unix utility will change the line endings for a file if needed.

Install with: sudo apt install dos2unix

Example of running dos2unix: dos2unix install_prerequisites.sh

Now re-run the bash script after converting the line endings
...
├── compile_protobuf_files.sh<br>
├── install_prerequisites.sh<br>
├── README.md<br>
├── usage_examples<br>
