...
void B_scan_extended_publisher::fft_data_handler(const Client& radar_client [[maybe_unused]], const Message& msg)
{
    ...
    timestamps_seconds[azimuth_index] = fft->timestamp_sec() * 1e9 + fft->timestamp_nsec();

    if (azimuth_index == (azimuth_samples / 2)) {
        latest_scan_ts = fft->timestamp_sec();
        latest_scan_ts_ns = fft->timestamp_nsec();
    }
