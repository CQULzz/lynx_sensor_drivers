...
void process_configuration(Client& radar_client, Message& msg)
{
    ...
    auto protobuf = from_vector_into<Core::Configuration::Protobuf::ConfigurationData>(cfg_msg->to_vector());
...
void target_handler(const Navtech::Navigation::Azimuth_target& target_data)
{
    syslog.write(
        "Target handler - "
        "Angle [" + std::to_string(target_data.angle) + "] "
        "Target count [" + std::to_string(target_data.targets.size()) + "] "
    );

    for (auto t : target_data.targets) {
        syslog.write(
            "\t"
            "Range [" + std::to_string(t.range) + "] "
            "Power [" + std::to_string(t.power) + "]"
        );
    }
}
