...
void Config_writer::on_write(const TCP::Message& msg)
{
    ...
    auto lens_str = [](Core::Configuration::Protobuf::LensTypes lens)
                    {
    ...
    syslog.write("Writing radar configuration...");
...
{
    syslog.write("Writing navigation configuration...");
...
{
    syslog.write("Writing configuration data to " + std::string { config_file });
...
int main(int argc, char* argv[])
{
    ...
    auto file_tag   = options.global("-f").value();
    ...
    syslog.write("Writing point-cloud data to " + data_file);
