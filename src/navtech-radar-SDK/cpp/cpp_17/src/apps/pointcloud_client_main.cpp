...
{
    syslog.write("Received keep-alive");
}
...
void process_point_cloud(Colossus_protocol::UDP::Client&, Colossus_protocol::UDP::Message& msg)
{
    ...
    syslog.linebreak();
    syslog.write("Azimuth:  " + std::to_string(spoke->azimuth()));
    syslog.write("Bearing:  " + spoke->bearing().to_string());
    ...
    for (std::size_t i { 0 }; i < sz; ++i) {
        auto& pt = points[i];
        log_stream << "[" << pt.range() << " m, " << pt.power() << " dB]";
    }
