...
namespace Navtech::Networking::Secure {
    ...
    Utility::Active::Task_state Receiver<protocol, transport, tls>::run()
    {
        ...
        try {
            ...
            syslog.debug("Secure receiver [" + std::to_string(id) + "] client disconnected.");
            ...
            syslog.debug("Secure receiver [" + std::to_string(id) + "] caught exception: " + e.what());
    ...
    {
        syslog.debug("Secure receiver [" + std::to_string(id) + "] shutdown. Requesting termination.");
    ...
    void Receiver<protocol, transport, tls>::disable()
    {
        if (!enabled) return;
        enabled = false;
        syslog.debug("Secure receiver [" + std::to_string(id) + "] disabled.");
    }
