...
namespace Navtech::Networking::Bytestream {
    ...
    {
        syslog.debug("Bytestream sender [" + std::to_string(id) + "] starting...");
    ...
    {
        syslog.debug("Bytestream sender [" + std::to_string(id) + "] stopping...");
    ...
    void Sender<protocol, transport, tls>::send(const Sender<protocol, transport, tls>::Message_buffer_Ty& msg)
    {
        ...
        try {
            ...
            syslog.debug("Bytestream sender [" + std::to_string(id) + "] caught exception: " + e.what());
    ...
    void Sender<protocol, transport, tls>::disable()
    {
        ...
        syslog.debug("Bytestream sender [" + std::to_string(id) + "] disabled");
    }
    ...
    {
        syslog.debug("Bytestream receiver [" + std::to_string(id) + "] starting...");
    ...
    {
        syslog.debug("Bytestream receiver [" + std::to_string(id) + "] stopping...");
    ...
    Active::Task_state Receiver<protocol, transport, tls>::run()
    {
        ...
        try {
            ...
            syslog.debug("Bytestream receiver [" + std::to_string(id) + "] client disconnected");            
        }
        catch (std::system_error& e) {
            syslog.debug("Bytestream receiver [" + std::to_string(id) + "] run() caught exception " + e.what());
    ...
    void Receiver<protocol, transport, tls>::disable()
    {
        ...
        syslog.debug("Bytestream receiver [" + std::to_string(id) + "] disabled");
    }
    ...
    {
        syslog.debug("Bytestream receiver [" + std::to_string(id) + "] signalling rx error");
        ...
        remote_endpt    { socket.peer() }
    {
        try {
            ...
            syslog.error(
                "Bytestream connection [" + std::to_string(ident) + "] "
                "error setting socket options: " + e.what()
            );
        }
    }
    ...
    void Connection<protocol, transport, tls>::open()
    {
        if (!socket.is_open()) {
            syslog.error(
                "Bytestream connection [" + std::to_string(ident) + "] "
                "cannot be opened [Socket is not open]"
            );
            return;
        }
        ...
        syslog.debug("Bytestream connection [" + std::to_string(ident) + "] open.")
    }
    ...
    void Connection<protocol, transport, tls>::close()
    {
        ...
        //    
        syslog.debug(
            "Bytestream connection [" + std::to_string(ident) + "] closed. "
    ...
    void Connection<protocol, transport, tls>::on_send_error(const ID_Ty& session_in_error)
    {
        ...
        //
        syslog.debug("Bytestream connection [" + std::to_string(ident) + "] tx error. Requesting termination.");
    ...
    void Connection<protocol, transport, tls>::on_receive_error(const ID_Ty& session_in_error)
    {
        ...
        // should terminate this connection.
        //
        syslog.debug("Bytestream connection [" + std::to_string(ident) + "] rx error. Requesting termination.");
    ...
    void Connection<protocol, transport, tls>::bind_to(const Endpoint& local_endpt)
    {
        ...
        try {
            ...
            syslog.error(
                "Bytestream connection [" + std::to_string(ident) + "] "
                "cannot bind to local endpoint [" + local_endpt.to_string() + "]"
            );
        }
    }
    ...
    void Connection<protocol, transport, tls>::remote_endpoint(const Endpoint& remote_endpt)
    {
        ...
        try {
            socket.remote_endpoint(remote_endpt);
             ...
             syslog.error(
                "Bytestream connection [" + std::to_string(ident) + "] "
                "cannot bind to local endpoint [" + remote_endpt.to_string() + "]"
            );
        }
    }
