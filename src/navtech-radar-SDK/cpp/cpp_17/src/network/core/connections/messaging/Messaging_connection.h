...
namespace Navtech::Networking::Messaging {
    ...
    {
        syslog.debug("Messaging sender [" + std::to_string(id) + "] starting...");
    ...
    {
        syslog.debug("Messaging sender [" + std::to_string(id) + "] stopping...");
    ...
    void Sender<protocol, transport, tls>::send(const Sender<protocol, transport, tls>::Message_buffer_Ty& msg)
    {
        ...
        try {
            ...
            syslog.debug("Messaging sender [" + std::to_string(id) + "] caught exception: " + e.what());
    ...
    {
        syslog.debug("Messaging sender [" + std::to_string(id) + "] enabled.");
        enabled = true;
    }
    ...
    void Sender<protocol, transport, tls>::disable()
    {
        ...
        syslog.debug("Messaging sender [" + std::to_string(id) + "] disabled.");
    }
    ...
    {
        syslog.debug("Messaging receiver [" + std::to_string(id) + "] starting...");
    ...
    {
        syslog.debug("Messaging receiver [" + std::to_string(id) + "] stopping...");
    ...
    void Receiver<protocol, transport, tls>::read_header()
    {
        ...
        try {
            ...
            //
            if (!enabled) {
                syslog.debug("Messaging receiver [" + std::to_string(id) + "] disabled after read.");
                return;
            }
            ...
            if (Protocol_traits::is_valid(incoming_msg)) {
                ...
                syslog.debug("Messaging receiver [" + std::to_string(id) + "] invalid header");
            ...
            syslog.debug("Messaging receiver [" + std::to_string(id) + "] remote endpoint disconnected.");
            ...
            syslog.debug(
                "Messaging receiver [" + std::to_string(id) + "] "
                "caught exception [" + std::to_string(e.code().value()) + "][" + e.what() + "]" 
            );
    ...
    {
        syslog.debug("Messaging receiver [" + std::to_string(id) + "] notifying rx error");
    ...
    {
        syslog.debug("Messaging receiver [" + std::to_string(id) + "] enabled");
        enabled = true;
    }
    ...
    void Receiver<protocol, transport, tls>::disable()
    {
        ...
        syslog.debug("Messaging receiver [" + std::to_string(id) + "] disabled");
    }
    ...
    {
        try {
            syslog.debug("Messaging connection [" + std::to_string(ident) + "] setting socket options...");

            Connection_traits::set_socket_options(socket);
        }
        catch (std::system_error& e) {
            syslog.debug(
                "Messaging connection [" + std::to_string(ident) + "] error setting socket options. "
                "Caught exception ["     + std::to_string(e.code().value()) + "][" + e.what() + "]"
            );
        }
    }
    ...
    {
        if (!socket.is_open()) {
            syslog.debug("Messaging connection [" + std::to_string(ident) + "] cannot be opened [Socket is not open]");
            return;
        }
        ...
        syslog.debug("Messaging connection [" + std::to_string(ident) + "] opened.");
    ...
    void Connection<protocol, transport, tls>::close()
    {
        ...
        //
        syslog.debug(
            "Messaging connection [" + std::to_string(ident) + "] closed. "
    ...
    void Connection<protocol, transport, tls>::on_send_error(const ID_Ty& session_in_error)
    {
        ...
        //
        syslog.debug("Messaging connection [" + std::to_string(ident) + "] tx error. Requesting termination.");
    ...
    void Connection<protocol, transport, tls>::on_receive_error(const ID_Ty& session_in_error)
    {
        ...
        // should terminate this connection.
        //
        syslog.debug("Messaging connection [" + std::to_string(ident) + "] rx error. Requesting termination.");
    ...
    void Connection<protocol, transport, tls>::bind_to(const Endpoint& local_endpt)
    {
        ...
        try {
            ...
            syslog.error(
                "Messaging connection [" + std::to_string(ident) + "] "
                "cannot bing to local endpoint [" + local_endpt.to_string() + "]"
            );
        }
    }
    ...
    void Connection<protocol, transport, tls>::remote_endpoint(const Endpoint& remote_endpt)
    {
        ...
        try {
            ...
            syslog.error(
                "Messaging connection [" + std::to_string(ident) + "] "
                "cannot bing to remote endpoint [" + remote_endpt.to_string() + "]"
            );
        }
    }
