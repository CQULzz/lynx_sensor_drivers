...
namespace Navtech::Networking {
    ...
    {
        syslog.debug("Radar client disconnected");
    ...
    void Stream_client<protocol, transport, tls>::connect()
    {
        ...
        try {
            syslog.info("Radar client - Connecting to [" + endpoint.to_string() + "]");
            ...
            syslog.error("Radar client - Connection failed to [" + endpoint.to_string() + "]");
    ...
    void Stream_client<protocol, transport, tls>::wait()
    {
        ...
        if (retry_count == retry_allowance) {
            syslog.error(
                "Radar client - tried to connect " + std::to_string(retry_count) + " times. "
        ...
        if (retry_allowance == std::numeric_limits<std::uint16_t>::infinity()) {
            syslog.info(
                "Radar client - Retry " + std::to_string(retry_count) + ". "
                "Trying again in " + wait_period.to_string()
            );
        }
        else {
            syslog.info(
                "Radar client - Retry " + std::to_string(retry_count) + " of " + std::to_string(retry_allowance) + ". "
                "Trying again in " + wait_period.to_string()
            );
        }
    ...
    void Stream_client<protocol, transport, tls>::open_connection()
    {
        ...
        syslog.debug("Radar client - connected");
    }
    ...
    {
        syslog.debug("Radar client - Initialising socket...");
