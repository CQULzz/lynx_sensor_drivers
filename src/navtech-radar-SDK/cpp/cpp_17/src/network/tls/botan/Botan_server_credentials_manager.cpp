...
namespace Navtech::Networking::TLS::Botan {
    ...
    bool Server_credentials_manager::load()
    {
        ...
        try {
            ...
            syslog.critical("TLS - Server credentials load failed: " + std::string { e.what() });
            return false;
        }
        ...
        try {
            ...
            while (!input.end_of_data()) {
                try {
                    ...
                    syslog.critical("TLS - failed to load certificate: " + std::string { e.what() });
        ...
        if (certificate_info.certificates.size() == 0) {
            syslog.critical("TLS - failed to load any certificates! ");
