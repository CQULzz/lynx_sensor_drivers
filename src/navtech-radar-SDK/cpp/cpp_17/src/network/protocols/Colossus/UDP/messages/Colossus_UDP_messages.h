...
#pragma pack(1)
    ...
    class IMU : public Message_base::Header_only<IMU> {
    public:
        struct IMU_values {
            // These are in mili-G from 0 to +-2G
            // The calibration data has NOT been applied to these
            int16_t x_acc { };
            int16_t y_acc { };
            int16_t z_acc { };
        
            // units of 0.1 degrees per second (signed). 
            // The calibration data has NOT been applied to these
            int16_t roll_vel  { };
            int16_t pitch_vel { };
            int16_t yaw_vel   { }; 
            
            // Processed radar angle data 
            // Phi (roll)    - Rotation around the radar's North South axis
            //                 +ve is roll to starboard        
            // Theta (Pitch) - Rotation around the radar's East-West axis.
            //                 +ve is radar 'nose down'
            // Psi (yaw)	 - Rotation about the radar head's axis of rotation
            //                 +ve is in the direction of gravity (down).
            //
            // These are in units of 0.1 degrees (signed) and the calibration 
        ...
        {
            auto ntp         = t.to_timespec();
            imu_payload.timestamp_sec    = to_uint32_network(static_cast<std::uint32_t>(ntp.tv_sec));
            imu_payload.timestamp_subsec = to_uint32_network(static_cast<std::uint32_t>(ntp.tv_nsec));
        }
        ...
        Time::Real_time::Observation timestamp() const
        {
            return Time::Real_time::Observation { 
                timespec { 
                    static_cast<time_t>(to_uint32_host(imu_payload.timestamp_sec)), 
                    static_cast<long>(to_uint32_host(imu_payload.timestamp_subsec)) 
                } 
            }; 
        }

        void encoder_count(const Unit::Encoder_step& s)
        {
            imu_payload.encoder_step = to_uint16_network(s);
        }

        Unit::Encoder_step encoder_count() const
        {
            return to_uint16_host(imu_payload.encoder_step);
        }


        void data(const IMU_values& imu_data)
        {
            // We're casting the signed 16bit values to unsigned for transmission.
            imu_payload.x_acc      = to_uint16_network(static_cast<uint16_t>(imu_data.x_acc));
            imu_payload.y_acc      = to_uint16_network(static_cast<uint16_t>(imu_data.y_acc));
            imu_payload.z_acc      = to_uint16_network(static_cast<uint16_t>(imu_data.z_acc));
            imu_payload.roll_vel   = to_uint16_network(static_cast<uint16_t>(imu_data.roll_vel));
            imu_payload.pitch_vel  = to_uint16_network(static_cast<uint16_t>(imu_data.pitch_vel));
            imu_payload.yaw_vel    = to_uint16_network(static_cast<uint16_t>(imu_data.yaw_vel));
            imu_payload.phi_angl   = to_uint16_network(static_cast<uint16_t>(imu_data.phi_angl));
            imu_payload.theta_angl = to_uint16_network(static_cast<uint16_t>(imu_data.theta_angl));
            imu_payload.psi_angl   = to_uint16_network(static_cast<uint16_t>(imu_data.psi_angl));
        }

        IMU_values data() const
        {
            return IMU_values {
                static_cast<int16_t>(to_uint16_host(imu_payload.x_acc)),
                static_cast<int16_t>(to_uint16_host(imu_payload.y_acc)),
                static_cast<int16_t>(to_uint16_host(imu_payload.z_acc)),
                static_cast<int16_t>(to_uint16_host(imu_payload.roll_vel)),
                static_cast<int16_t>(to_uint16_host(imu_payload.pitch_vel)),
                static_cast<int16_t>(to_uint16_host(imu_payload.yaw_vel)),
                static_cast<int16_t>(to_uint16_host(imu_payload.phi_angl)),
                static_cast<int16_t>(to_uint16_host(imu_payload.theta_angl)),
                static_cast<int16_t>(to_uint16_host(imu_payload.psi_angl)),
            };
        }
        std::size_t size() const 
        { 
            return sizeof(imu_payload); 
        }
    
    private:
    struct {
        std::uint32_t timestamp_sec;
        std::uint32_t timestamp_subsec;

        std::uint16_t encoder_step;

        std::uint16_t x_acc;
        std::uint16_t y_acc;
        std::uint16_t z_acc;
        std::uint16_t roll_vel;
        std::uint16_t pitch_vel;
        std::uint16_t yaw_vel;
        std::uint16_t phi_angl;
        std::uint16_t theta_angl;
        std::uint16_t psi_angl;
    } imu_payload;
    };
