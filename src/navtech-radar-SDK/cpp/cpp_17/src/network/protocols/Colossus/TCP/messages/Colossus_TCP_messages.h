...
#pragma pack(1)
    ...
    public:
        ...
        std::uint32_t timestamp_sec() const                     { return sec; }
        void timestamp_sec(std::uint32_t val)                   { sec = val; }
                
        std::uint32_t timestamp_nsec() const                    { return nsec; }
        void timestamp_nsec(std::uint32_t val)                  { nsec = val; }
                
        Time::Real_time::Observation timestamp() const          {
                                                                    timespec t_val { 
                                                                        static_cast<long>(timestamp_sec()),
                                                                        static_cast<long>(timestamp_nsec())
                                                                    };
                                                                    return Time::Real_time::Observation { t_val };
                                                                }

        void timestamp(const Time::Real_time::Observation& t)   {
                                                                    auto ts = t.to_timespec();
                                                                    timestamp_sec(static_cast<uint32_t>(ts.tv_sec));
    ...
    public:
        ...
        void now(const Time::Real_time::Observation& t) { 
                                                            now_sec  = to_uint32_network(static_cast<std::uint32_t>(t.to_timespec().tv_sec));
                                                            now_nsec = to_uint32_network(static_cast<std::uint32_t>(t.to_timespec().tv_nsec));
                                                        }
